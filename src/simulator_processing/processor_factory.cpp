...
std::unique_ptr<ProcessorBase> ProcessorFactory::createFromRos(
    const std::string& name, const std::string& type, const ros::NodeHandle& nh,
    const std::string& ns, AirsimSimulator* parent) {
  if (getRegistryMap().find(type) == getRegistryMap().end()) {
    ...
    LOG(ERROR) << "Unknown type '" << type << "' for simulator_processor '"
               << name << "'. Available types are: " << typeList << ".";
    return nullptr;
  }
  ...
  if (!processor->setupFromRos(nh, ns)) {
    LOG(ERROR) << "Setup for simulator_processor '" << name
               << "' failed, it will be ignored.";
    return nullptr;
  }
