...
class ConfigParser(object):
    def __init__(self):
        ...
        if self.target_file_path == "":
            self.target_file_path = os.path.join(os.path.expanduser("~"),
                                                 "Documents", "AirSim",
                                                 "settings.json")
        self.config_file_path = rospy.get_param(
            '~config_file_path', "no_source_path_given")  # Where the source is

        # Initial / minimal config
        self.new_cfg = {
            "ConfigParserInfo":
            "This config was auto-created from source '%s' on %s." %
        ...
        self.np_general = [
            "SeeDocsAt", "SettingsVersion", "SimMode", "ConfigParserInfo"
        ]
        self.np_camera = [
            "T_B_S", "X", "Y", "Z", "Pitch", "Roll", "Yaw", "CaptureSettings"
        ]
        self.image_type_map = {
            "Scene": 0,
            "DepthPlanar": 1,
    ...
    def parse(self):
        ...
        info = "* Starting config parser for unreal_airsim to AirSim config " \
               "conversion *"
        self.log("\n" + "*" * len(info) + "\n" + info + "\n" + "*" * len(info))
    ...
    def parse_sensors(self):
        ...
        for name in sensors:
            ...
            if not "sensor_type" in data:
                self.log(
                    "Skipping sensor '%s', no 'sensor_type' given!" % name, 2)
                continue
            ...
            if sensor_type == "Camera":
                ...
                if image_type not in self.image_type_map:
                    self.log(
                        ...
                        "(Known types are %s)" % (image_type, ', '.join(
                            " %s" % self.image_type_map[key]
                            for key in self.image_type_map)), 1)
                    image_type = "Scene"
                capture_settings = {
                    "ImageType": self.image_type_map[image_type]
                }
    ...
    def read_source_config(self):
        ...
        if not os.path.isfile(self.config_file_path):
            self.log(
                "Source file '%s' could not be found!" % self.config_file_path,
                2)
            sys.exit()
        try:
            ...
            self.log(
                "Source file '%s' is not a valid yaml file and could not be "
                "read!" % self.config_file_path, 2)
            sys.exit()
    ...
    def setup_target_file(self):
        ...
        if backup_old_file:
            backup_name = self.target_file_path + datetime.now().strftime(
                ".backup_%d_%m_%Y_%H_%M_%S")
            os.rename(self.target_file_path, backup_name)
            self.log("Created backup of existing settings at '%s'." %
                     backup_name)
    ...
    def forward_args(in_dict, out_dict, exclusion):
        ...
        for key in in_dict:
            if str(key[:1]).isupper():
                # Airsim settings are CamelCase, unreal_airsim are lower_case
                if key not in exclusion:
                    out_dict[key] = in_dict[key]

    def parse_transformation(self, list_in, dict_out):
        if list_in is None:
            dict_out["X"] = 0
            dict_out["Y"] = 0
            dict_out["Z"] = 0
            ...
            if len(list_in) != 4:
                self.log(
                    "Transformations are expected as 4x4 matrices, data "
                    "will be ignored!", 1)
                ...
                for l in list_in:
                    if len(l) != 4:
                        self.log(
                            "Transformations are expected as 4x4 matrices,"
                            " data will be ignored!", 1)
                        self.parse_transformation(None, dict_out)
                        return
