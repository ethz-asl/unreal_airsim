...
/***
 * Reads an image of image type 'Infrared' and shifts the values s.t. they match the object segmentation ID.
 * This is necessary as apparently for some setups that translation fails: https://github.com/microsoft/AirSim/issues/1992
*/
class InfraredIdCompensation : public ProcessorBase {
 ...
 protected:
  ...
  // the actual compensation values as measured for the current setup. Values that can not be mapped to are 255,
  // so if anything goes wrong it can be addressed via this special id label.
...
#endif // UNREAL_AIRSIM_SIMULATOR_PROCESSOR_INFRARED_ID_COMPENSATION_H_
