...
/***
 * Static Factory with a singleton registration map. Processors can register themselves with a name using the static
 * Registration struct, such that they can then be created via the name at runtime.
*/
class ProcessorFactory {
 public:
  ...
  // Allow processors to register themselves to the factory using a static Registration struct
  template<class ProcessorType>
  struct Registration {
    Registration(const std::string &name) {
      if (getRegistryMap().find(name) == getRegistryMap().end()) {
        getRegistryMap().insert(std::pair<std::string, FactoryMethod>(name,
                                                                      []() { return new ProcessorType(); }));
      } else {
        LOG(FATAL) << "Cannot register already existing simulator_processor name '" << name << "'!";
      }
    }
  };
